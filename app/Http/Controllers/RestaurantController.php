...
class RestaurantController extends Controller
{
    ...
    public function index()
    {
        ...
        // $data = Employee::where('branch', $branch)->get();
        ...
        $categories = Category::where('type', 'Restaurant')
            ->where('branch', $branch)
            ->get();
        $meals = Meal::where('branch', $branch)->get();

        // $orders = Order::where('branch', $branch)
        // ->where('status', 'pending')
        // ->get();
        // $orders = Order::where('orders.branch', $branch)
        // ->where('orders.status', 'pending')
        // ->join('order_items', 'orders.id', '=', 'order_items.order_id') // Join with order_items table
        // ->join('meals', 'order_items.itemable_id', '=', 'meals.id') // Join with meal table using itemable_id
        // ->select('orders.*', 'order_items.total', 'order_items.itemable_id', 'meals.name') // Select the fields
        // ->get();

        $orders = Order::where('orders.branch', $branch)
        ->where('orders.status', 'pending')
        ->with(['items.meal']) // Load order items and their related meal data
        ->get();


        return view('restaurant.index', compact('title', 'breadcrumbs', 'categories', 'meals','orders'));
    }
    ...
    public function checkout(Request $request)
    {
        ...
        try {
            ...
            return response()->json(['success' => true, 'message' => 'Order Placed!', 'url' => route('order.print', [$order])]);
        } catch (\Throwable $th) {
            //throw $th;
